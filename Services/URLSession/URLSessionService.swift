...
public final class URLSessionService {
    ...
    init(mockServer: (host: String?, queryParams: [String: String]?)? = nil) {
...
extension URLSessionService: FreeURLSessionServiceProtocol {
    
    func fetchJSON<Endpoint: URLSessionServiceEndpoint>(endpoint: Endpoint) async throws -> Endpoint.Response {
        guard let request = endpoint.request else { throw CAError.requestCreate }
        do {
            return try await fetchJSON(withEndpoint: endpoint)
        } catch {
            return try await fallbackRefreshToken(request: request, networkError: error)
        }
    }
    
    func fetchJSON<T: Codable>(request: URLRequest?) async throws -> T {
        guard let request else { throw CAError.requestCreate }
        do {
            return try await fetchJSON(request: request, url: nil)
        } catch {
            return try await fallbackRefreshToken(request: request, networkError: error)
        }
    }
    ...
    func fetchData(request: URLRequest?) async throws -> (Data, URLResponse) {
        guard let request else { throw CAError.requestCreate }
        do {
            return try await fetchData(request: request, url: nil)
        } catch {
            return try await fallbackRefreshToken(request: request, networkError: error)
        }
    }
    
    private func fallbackRefreshToken<T: Codable>(request: URLRequest, networkError: Error) async throws -> T {
        if [403].contains(networkError.getURLSessionStatusCode) {
            if request.allHTTPHeaderFields?["X-Free-Console-AT"] != nil { // Freebox
                var newAccessToken: String?
                do {
                    let refreshTokenUsecase = FbxAuthRefreshTokenUsecase()
                    let result = try await refreshTokenUsecase.execute(input: .init())
                    newAccessToken = result.newAccessToken
                } catch {
                    throw networkError
                }
                var newRequest = request
                newRequest.allHTTPHeaderFields?["X-Free-Console-AT"] = newAccessToken
                return try await fetchJSON(request: newRequest, url: nil)
            }
            if request.allHTTPHeaderFields?["Authorization"] != nil { // BU
                var newAccessToken: String?
                do {
                    let refreshTokenUsecase = MobAuthRefreshTokenUsecase()
                    let result = try await refreshTokenUsecase.execute(input: .init())
                    newAccessToken = result.newAccessToken
                } catch {
                    throw networkError
                ...
                var newRequest = request
                if let newToken = newAccessToken {
                    newRequest.allHTTPHeaderFields?["Authorization"] = "Bearer \(newToken)"
                }
                return try await fetchJSON(request: newRequest, url: nil)
            }
            if request.allHTTPHeaderFields?["x-user-token"] != nil { // BFF
                var newAccessToken: String?
                do {
                    let refreshTokenUsecase = MobAuthRefreshTokenUsecase()
                    let result = try await refreshTokenUsecase.execute(input: .init())
                ...
                var newRequest = request
                if let newToken = newAccessToken {
                    newRequest.allHTTPHeaderFields?["x-user-token"] = newToken
                }
                return try await fetchJSON(request: newRequest, url: nil)
            }
        }
    ...
    private func fallbackRefreshToken(request: URLRequest, networkError: Error) async throws -> (Data, URLResponse) {
        if [403].contains(networkError.getURLSessionStatusCode) {
            if request.allHTTPHeaderFields?["X-Free-Console-AT"] != nil { // Freebox
                var newAccessToken: String?
                do {
                ...
                var newRequest = request
                newRequest.allHTTPHeaderFields?["X-Free-Console-AT"] = newAccessToken
                return try await fetchData(request: newRequest, url: nil)
            }
            if request.allHTTPHeaderFields?["Authorization"] != nil { // BU
                var newAccessToken: String?
                do {
