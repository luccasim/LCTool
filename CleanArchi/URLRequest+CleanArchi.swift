...
extension String {
    var toRequest: URLRequest? {
        URL(string: self).flatMap({URLRequest(url: $0)})
    }
    ...
    func toRequest(
        ...
        header: [String: String] = [:],
        bodyForm: URLRequest.BodyForm? = .none
    ) -> URLRequest? {
        URLRequest(path: self, queries: queries, httpMethod: httpMethod, header: header, bodyForm: bodyForm)
    }
}
